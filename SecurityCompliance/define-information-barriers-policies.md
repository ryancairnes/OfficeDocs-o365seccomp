...
5. Define policies by using PowerShell cmdlets, such as used in the following examples. Select a scenario, and then run the cmdlets one at a time. The first two cmdlets identify the users who are affected. The next two define variables for your information barrier policy, and the last defines the policy using those variables.<br/>
